...
export default function ProfilePage() {
  ...
  const navigateToProfile = async (userId) => {
    try {
      ...
      if (!res.ok) {
        console.error(
          "❌ Failed to fetch profile for navigation:",
          res.status,
          res.statusText
        );
  ...
  const [newLang, setNewLang] = useState({
    languageName: "",
    languageProficiency: LANGUAGE_PROFICIENCY_OPTIONS[0].value,
  });
    ...
    try {
      ...
      const connectionPromises = connections.map(async (connection) => {
        try {
          ...
          const otherUserId =
            String(connection.senderId) === String(userId)
              ? connection.receiverId
              : connection.senderId;

          console.log(
            `🧩 Processing connection: ${connection.id}, otherUserId: ${otherUserId}`
          );
          console.log(
            `   - sender: ${connection.senderId}, receiver: ${connection.receiverId}`
          );
          ...
          if (!userRes.ok) {
            console.error(
              `❌ Failed to fetch profile for user ${otherUserId}: ${userRes.status}`
            );
            return {
              ...connection,
              name: "Unknown User",
          ...
          return {
            ...
            name:
              ...
              "Unknown User",
            profilePictureUrl:
              userData.profilePictureUrl || "/default-avatar.png",
      ...
      console.log(
        "✅ Final connections with profiles:",
  ...
  useEffect(() => {
    if (profile && profile.userId) {
      console.log(
        "🔄 Profile loaded, fetching related data for userId:",
        profile.userId
      );
      fetchExperiences();
      fetchEducations();
  ...
  async function fetchSkills() {
    ...
    setSkillError("");
    try {
      const res = await fetch(`${SKILL_ROOT}?ProfileId=${profile.id}`, {
        credentials: "include",
      });
      if (!res.ok) throw new Error("Yetenekler yüklenemedi.");
      const data = await res.json();
  ...
  async function handleSaveSkill(e) {
    ...
    try {
      ...
      if (newSkill.proficiencyValue < 1 || newSkill.proficiencyValue > 5) {
        throw new Error("Lütfen bir yeterlilik seviyesi seçin.");
      }
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData?.error?.message || "Yetenek kaydedilemedi.");
      }
  ...
  async function fetchLanguages() {
    ...
    setLangError("");
    try {
      const res = await fetch(`${LANGUAGE_ROOT}?ProfileId=${profile.id}`, {
        credentials: "include",
      });
  ...
  function openEditLang(lang) {
    ...
    const proficiencyString =
      LANGUAGE_PROFICIENCY_OPTIONS[Number(lang.languageProficiency)]?.value ||
      "Elementary";
  ...
  async function handleSaveLang(e) {
    ...
    try {
      ...
      if (editingLang && currentLang) {
        res = await fetch(`${LANGUAGE_ROOT}/${currentLang.id}`, {
          method: "PUT",
          credentials: "include",
          headers: {
            accept: "application/json",
      ...
      if (!res.ok) throw new Error("Dil kaydı başarısız");
  ...
  async function fetchProjects() {
    ...
    setProjError("");
    try {
      const res = await fetch(`${PROJECT_ROOT}?ProfileId=${profile.id}`, {
        credentials: "include",
      });
  ...
  async function handleSaveProj(e) {
    ...
    try {
      ...
      if (editingProj && currentProj) {
        res = await fetch(`${PROJECT_ROOT}/${currentProj.id}`, {
          method: "PUT",
          credentials: "include",
          headers: {
            accept: "application/json",
      ...
      if (!res.ok) throw new Error("Proje kaydı başarısız");
  ...
  async function fetchExperiences() {
    ...
    setExpError("");
    try {
      const res = await fetch(`${EXPERIENCE_ROOT}?ProfileId=${profile.id}`, {
        credentials: "include",
      });
  ...
  async function fetchEducations() {
    ...
    setEduError("");
    try {
      const res = await fetch(`${EDUCATION_ROOT}?ProfileId=${profile.id}`, {
        credentials: "include",
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setProfile(updated);
      setForm({
        // Update form state with potentially new data from backend
        about: updated.about || "",
        email: updated.email || "",
          ...
          <div className="profile-card">
            ...
            <>
              ...
              {(profile?.name || profile?.surname || profile?.userName) && ( // Show this section if name or username exists
                <div className="profile-actual-name">
                  {" "}
                  {/* Keep existing class */}
                  {(profile?.name || profile?.surname) &&
                    !currentUser?.isCompanyProfile && ( // Show individual name if exists and not company
                      <span>
                        {`${profile?.name || ""} ${
                          profile?.surname || ""
                        }`.trim()}
                  ...
                  {profile?.name &&
                    currentUser?.isCompanyProfile && ( // Show company name if exists and is company
              ...
              {(profile?.email || profile?.phoneNumber || profile?.address) && ( // Only show if any contact info exists
                <div className="profile-contact-section">
                  ...
                  <div className="contact-info-list">
                    {" "}
                    {/* Inner container for consistent layout */}
                    {profile?.email && ( // Email
                      <div>
                        <Mail size={20} />
              ...
              <div className="profile-info-list">
                ...
                {!currentUser?.isCompanyProfile &&
                  profile?.birthday && ( // Keep birthday here
                    <div>
                      <Calendar size={20} />
                      <span>
        ...
        {showConnections && (
          ...
          >
            ...
            >
              ...
              {connectionsError && (
                <div className="error-message">{connectionsError}</div>
              )}
                ...
                <div className="connections-grid">
                  {connections.length === 0 ? (
                    ...
                    connections.map((connection) => {
                      const otherUserId =
                        String(connection.senderId) === String(profile?.userId)
                          ? connection.receiverId
                          : connection.senderId;
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            >
              ...
              <input
                ...
                name="name"
                placeholder={
                  currentUser?.isCompanyProfile ? "Şirket Adı" : "Ad"
                }
                value={form.name}
                onChange={handleChange}
                required
        ...
        <div className="left-column">
                ...
                <>
                  <div className="experiences-grid">
                    {experiences
                      .slice(
                        0,
                        expandedSections.experiences ? experiences.length : 2
                      )
                      .map((exp) => (
          ...
          {!currentUser?.isCompanyProfile && (
            <section className="education-section">
              ...
              {eduError && <div className="error-message">{eduError}</div>}
              {eduLoading ? (
                <div>Yükleniyor...</div>
              ) : (
                <>
                  <div className="experiences-grid">
                    {educations
                      .slice(
                        0,
                        expandedSections.education ? educations.length : 2
                      )
                      .map((ed) => (
                        <div className="experience-card" key={ed.id}>
                          <div className="experience-card-header">
                            <span className="experience-title">
                              {ed.instutionName}
                            </span>
                ...
                <>
                  <div className="languages-grid">
                    {languages
                      .slice(
                        0,
                        expandedSections.languages ? languages.length : 2
                      )
                      .map((l) => {
      ...
      {showExpModal && (
        <div className="modal-overlay" onClick={() => setShowExpModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <form className="experience-form" onSubmit={handleSaveExp}>
              <h3>{editingExp ? "Deneyimi Düzenle" : "Yeni Deneyim Ekle"}</h3>
              <input
                type="text"
                name="title"
              ...
              <button
                ...
                style={{ marginTop: 8 }}
              >
                <Save size={18} /> {editingExp ? "Güncelle" : "Ekle"}
              </button>
            </form>
          </div>
        </div>
      ...
      {showEduModal && (
        <div className="modal-overlay" onClick={() => setShowEduModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <form className="experience-form" onSubmit={handleSaveEdu}>
              <h3>{editingEdu ? "Eğitimi Düzenle" : "Yeni Eğitim Ekle"}</h3>
              <input
                type="text"
                name="instutionName"
              ...
              <button
                ...
                style={{ marginTop: 8 }}
              >
                <Save size={18} /> {editingEdu ? "Güncelle" : "Ekle"}
              </button>
            </form>
          </div>
        </div>
                  ...
                  {[1, 2, 3, 4, 5].map((n) => (
                    <button
                      ...
                      className={`dot-button ${
                        n <= newSkill.proficiencyValue ? "filled" : ""
                      }`}
      ...
      {showProjModal && (
        <div className="modal-overlay" onClick={() => setShowProjModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <form className="experience-form" onSubmit={handleSaveProj}>
              <h3>{editingProj ? "Projeyi Düzenle" : "Yeni Proje Ekle"}</h3>
              <input
                type="text"
                name="projectName"
              ...
              />
              <button type="submit" className="save-profile-btn">
                <Save size={18} /> {editingProj ? "Güncelle" : "Ekle"}
              </button>
              {projError && <div className="error-message">{projError}</div>}
            </form>
          </div>
