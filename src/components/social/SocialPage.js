...
function UserCard({ user, onConnect, connectionStatus }) {
  ...
  const getConnectionButton = () => {
    ...
    switch (status) {
      case "connected":
        return (
          <span className="connection-badge connected">Bağlantı kuruldu</span>
        );
      case "pending-outgoing":
        return <span className="connection-badge pending">Beklemede</span>;
      case "none":
        return (
  ...
  return (
    <div
      className={`user-card ${user.isCompanyProfile ? "company-profile" : ""}`}
    ...
    >
      ...
      <div className="user-card-footer">{getConnectionButton()}</div>
    </div>
  );
}
...
function SocialPage() {
      ...
      const userDetails = await Promise.all(
        users.map(async (profile) => {
          ...
          try {
            ...
            console.log(
              `Error fetching skills for user ${profile.id}: ${err.message}`
            );
  ...
  const fetchUserProfile = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/app/user-profile/by-user`, {
        credentials: "include",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
                  ...
                  parts: [
                    {
                      text: `
                        ...
                        Please return ONLY the following JSON object, without any code block or extra text give the match reason in the Turkish language:
                        {
                        "matches": [
                            {
      ...
      const usersWithDetails = await Promise.all(
        profiles.map(async (profile) => {
          ...
          if (expRes.ok) {
            ...
            latestExperience = experiences.sort(
          ...
          if (eduRes.ok) {
            ...
            latestEducation = educations.sort(
              (a, b) => new Date(b.startDate) - new Date(a.startDate)
            )[0];
          }
  ...
  async function fetchConnections() {
    ...
    try {
      ...
      const incomingRes = await fetch(
        `${CONNECTION_ROOT}/incoming-list?SkipCount=0&MaxResultCount=100`,
        {
          credentials: "include",
          headers: {
            Accept: "application/json",
      ...
      const outgoingRes = await fetch(
        `${CONNECTION_ROOT}/outgoing-list?SkipCount=0&MaxResultCount=100`,
        {
          credentials: "include",
          headers: {
            Accept: "application/json",
      ...
      const allConnectionsRes = await fetch(
        `${CONNECTION_USER_ROOT}?SkipCount=0&MaxResultCount=1000`,
        {
          credentials: "include",
          headers: {
            Accept: "application/json",
      ...
      if (allConnectionsRes.ok) {
        ...
        const acceptedConnections = (allConnectionsData.items || []).filter(
          (c) => c.status === 1
        );
  ...
  async function sendConnectionRequest(receiverUserId) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Bu kullanıcıya daha önce bağlantı isteği gönderdiniz!`
        );
      }
  ...
  async function respondToRequest(connectionId, status) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to respond to connection request (Status: ${response.status})`
        );
      }
  ...
  function getConnectionStatus(userAbpId) {
    ...
    const acceptedConnection = connections.find(
      (c) =>
        (c.senderId === userAbpId || c.receiverId === userAbpId) &&
        c.status === 1
    );
    ...
    const outgoingPending = outgoingRequests.find(
      (c) => c.receiverId === userAbpId && c.status === 0
    );
    if (outgoingPending) {
    ...
    const incomingPending = incomingRequests.find(
      (c) => c.senderId === userAbpId && c.status === 0
    );
    if (incomingPending) {
        ...
        <div className="social-sidebar">
          ...
          <section className="profile-filter-section">
            ...
            <div className="profile-filter-options">
              ...
              <button
                className={`filter-btn ${
                  profileTypeFilter === "individual" ? "active" : ""
                }`}
              ...
              <button
                className={`filter-btn ${
                  profileTypeFilter === "company" ? "active" : ""
                }`}
          ...
          <section className="incoming-requests-section">
            <h2>
              Gelen Bağlantı İstekleri ({pendingIncomingRequests.length || 0})
            </h2>
                      ...
                      <div className="request-info">
                        ...
                        {sender.latestExperience && (
                          <p>
                            ...
                            {sender.latestExperience.title} @{" "}
                            {sender.latestExperience.companyName}
                          </p>
                        )}
                      </div>
            ...
            <div className="recommendations-list">
              {connectionsLoading ? (
                ...
                suggestedConnections.map((suggestion) => {
                  ...
                  const suggestedUser = users.find(
                    (u) => u.id === suggestion.id
                  );
                  ...
                  return (
                    <div key={suggestion.id} className="recommendation-card">
                      <img
                        src={
                          suggestion.profilePictureUrl || "/default-avatar.png"
                        }
                        alt={suggestion.name}
