...
function EducationPage() {
  ...
  const checkInstructorStatus = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error("Eğitmen durumu kontrol edilemedi.");
      }
    } catch (err) {
      console.error("Eğitmen durumu kontrol hatası:", err);
  ...
  const fetchCourses = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Kurslar alınamadı (Durum: ${response.status})`);
      }
